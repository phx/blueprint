...
def test_basis_functions(basis):
    ...
    # Scaling is validated through projection coefficients; normalized modes
    # should remain finite and nonzero on the fixed grid.
    max_amp = np.max(np.abs(functions[-1].psi))
