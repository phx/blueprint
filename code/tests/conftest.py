...
from core.types import Energy, Momentum, CrossSection, FieldConfig, NumericValue, WaveFunction
...
def quantum_state():
    def _generate(n: int = 0, l: int = 0):
        ...
        psi = np.exp(-grid**2 / 2)
        wf = WaveFunction(psi=psi, grid=grid, mass=1.0, quantum_numbers={'n': n, 'l': l, 'E': n + 1})
        wf.normalize()
        return wf
    return _generate
