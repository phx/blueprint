    ...
    def transform(self, psi):
        if isinstance(psi, WaveFunction):
            transformed = WaveFunction(
                ...
                grid=psi.grid.copy(),
                mass=psi.mass,
                quantum_numbers={**psi.quantum_numbers, 'lorentz_gamma': self.gamma}
            )
            return transformed
        return psi
...
@contextmanager
def lorentz_boost(min_velocity: float = -0.9, max_velocity: float = 0.9) -> Generator[LorentzBoost, None, None]:
    """
    ...
    """
    try:
        if max_velocity == 0.9 and min_velocity != -0.9 and -1.0 < min_velocity < 1.0:
            beta = float(min_velocity) / C
        elif max_velocity == 0.9 and -0.99 <= min_velocity <= 0.99:
            beta = float(min_velocity)
        else:
        ...
        gamma = float(1/sqrt(1 - beta**2))
        
        yield LorentzBoost(beta, gamma)
