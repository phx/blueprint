...
class PrecisionMeasurement:
    ...
    def _compute_uncertainty(self, value: float) -> float:
        scale = max(abs(float(value)), 1.0)
        energy_scale = getattr(self, '_last_energy', 1.0)
        return scale * 1e-15 * max(float(energy_scale), 1.0)
...
def validate_precision(
    ...
    return (
        rel_error <= rtol and
        abs_error <= atol and
        value.uncertainty <= max(atol, rtol * (abs(target) + atol))
    )
