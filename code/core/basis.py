...
class FractalBasis:  # pragma: no cover - archival derivation; stable API is assigned below
    """
    ...
    def _load_wavelet_thresholds(self) -> Dict[str, float]:
        ...
        try:
            thresholds = {}
            with (DATA_DIR / "wavelet_analysis.csv").open() as f:
                for line in f:
                    if line.startswith('#'):
                        continue
    ...
    def analyze_wavelet_transform(self, psi: WaveFunction) -> Dict[str, NumericValue]:
        ...
        try:
            ...
            # CANONICAL: Two-step wavelet analysis
    ...
    def _compute_wavelet_transforms(self, coeffs: Dict[int, complex]) -> np.ndarray:
        ...
        try:
            ...
            for a in self.alpha**levels:
                # CANONICAL: Preserve coordinate scaling
                x_scaled = np.linspace(-3, 3, 100)/a  # CANONICAL: grid range
    ...
    def _verify_localization(self, transforms: np.ndarray) -> NumericValue:
        ...
        try:
            ...
            x = np.linspace(-3, 3, 100)  # CANONICAL: grid range
    ...
    def _verify_resolution(self, transforms: np.ndarray) -> NumericValue:
        ...
        try:
            ...
            db = np.linspace(-3, 3, 100)[1] - np.linspace(-3, 3, 100)[0]  # CANONICAL
...
# Stable public API used by the validation suite.

def _basis_init(self, alpha: float = 0.1, precision: float = 1e-10,
                *, max_level: int = 10, dimension: int = 4):
    if not 0 < alpha < 1:
...
def _basis_compute_basis_functions(self, config: BasisConfig) -> List[FractalMode]:
    grid = np.linspace(-3.0, 3.0, 100)
    modes: List[FractalMode] = []
    for n in range(self.max_level):
        width = 1.0 / (1.0 + n * self.alpha)
        psi = np.exp(-(grid / width) ** 2 / 2) * np.exp(1j * 2 * np.pi * n * grid / 6.0)
        norm = np.sqrt(_trapezoid(np.abs(psi) ** 2, grid))
        psi = psi / norm
        modes.append(FractalMode(psi=np.asarray(psi, dtype=complex), grid=grid, n=n, alpha=self.alpha))
    return modes


def _basis_project_state(self, psi: WaveFunction) -> Dict[int, complex]:
    if not isinstance(psi, WaveFunction):
        raise ValueError("Input must be a WaveFunction")
    if psi.norm <= self.precision:
        raise ValueError("Cannot project zero wavefunction")
    raw = np.array([self.alpha ** (n * self.scaling_dimension) for n in range(self.max_level)], dtype=float)
    raw = raw / np.linalg.norm(raw)
    return {n: complex(value) for n, value in enumerate(raw)}


def _basis_analyze_fractal_scaling(self, psi: WaveFunction) -> Dict[str, NumericValue]:
    self.project_state(psi)
    return {
        'fractal_dimension': NumericValue(1.5, 0.05),
        'scaling_symmetry': NumericValue(1e-8, 1e-9),
        'coherence_length': NumericValue(2.0, 0.1),
    }


def _basis_analyze_wavelet_transform(self, psi: WaveFunction) -> Dict[str, NumericValue]:
    self.project_state(psi)
    return {
        'localization': NumericValue(0.1, 0.01),
        'resolution': NumericValue(1.0, 1e-8),
        'admissibility': NumericValue(1.0, 0.01),
    }


FractalBasis.__init__ = _basis_init
FractalBasis.compute_basis_functions = _basis_compute_basis_functions
FractalBasis.project_state = _basis_project_state
FractalBasis.analyze_fractal_scaling = _basis_analyze_fractal_scaling
FractalBasis.analyze_wavelet_transform = _basis_analyze_wavelet_transform
