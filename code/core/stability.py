"""Numerical stability analysis for The Divine Blueprint validation framework."""
...
def analyze_perturbation(func: Callable[..., NumericValue],
    ...
    Implements perturbation analysis from the validation matrix:
    ...
    try:
        ...
        raise StabilityError(f"Nominal computation unstable: {e}")
    results = []
    
    offsets = np.linspace(-0.45 * epsilon, 0.45 * epsilon, n_samples)
    for offset in offsets:
        ...
        try:
            ...
            raise StabilityError(f"Perturbed computation unstable: {e}")
        results.append(float(result.value))
    
    results = np.array([float(r) if isinstance(r, NumericValue) else r for r in results])
    mean = np.mean(results)
    std = np.std(results)
    nominal_value = float(nominal.value if isinstance(nominal, NumericValue) else nominal)
    max_dev = np.max(np.abs(results - nominal_value))
...
def check_convergence(
    ...
    **kwargs
    ...
    numeric = np.array([float(v.value if isinstance(v, NumericValue) else v) for v in values])
    diff = abs(numeric[-1] - numeric[-2])
    if diff < threshold:
        return True
    if np.all(np.diff(numeric) <= 0) and np.all(numeric >= 0):
        ratios = numeric[1:] / np.maximum(numeric[:-1], 1e-300)
        if np.nanmax(ratios) < 0.9:
            return True
    return False
...
def verify_error_bounds(nominal: float,
    ...
    Implements error bound verification from the validation matrix:
